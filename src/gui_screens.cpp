...
void GUI::renderVariableSelection() {
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::BeginTooltip();
        ImGui::TextWrapped("Number of seasonal periods to use. For example, if set to 4 for quarterly data, the value from the same quarter in previous years will be included as features.");
        ImGui::EndTooltip();
    }
        ...
        if (!isTraining_) {
            ...
            trainingThread_ = std::thread([this]() {
                ...
                // and ensure the number matches the model input matrix
                {
                    ...
                    // Trim featureNames_ to match input matrix columns, if needed
                    if (featureNames_.size() > static_cast<size_t>(X.cols())) {
                        LOG_WARNING("Trimming feature names from " + std::to_string(featureNames_.size()) + 
                ...
                if (modelTrainingSuccess_) {
                    ...
                    if (modelCoefficients_.size() < featureNames_.size()) {
                        ...
                        // Create string of all feature names for debugging
                        std::string featureNamesList;
                        for (const auto& name : featureNames_) {
                            featureNamesList += name + ", ";
                        }
                        LOG_DEBUG("Feature names: " + featureNamesList);
                    }
        ...
        if (selectedModelIndex_ == 0 || selectedModelIndex_ == 1) { // Linear Regression or Elastic Net
            if (ImGui::CollapsingHeader("Model Coefficients", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (lagValues_ > 0 || seasonality_ > 0) {
                    ...
                    if (seasonality_ > 0) {
                        ImGui::Text("Seasonality period: %d", seasonality_);
                    }
                    ImGui::Separator();
                }

                // Debug: Show feature and coefficient counts with clear warning if they don't match
                if (featureNames_.size() != static_cast<size_t>(modelCoefficients_.size())) {
                    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.3f, 0.3f, 1.0f)); // Red text
                    ImGui::Text("Warning: Feature count (%zu) doesn't match coefficient count (%lld)", 
                               featureNames_.size(), modelCoefficients_.size());
                    ImGui::PopStyleColor();
                } else {
                    ImGui::Text("Total features/coefficients: %zu", featureNames_.size());
                }
                ...
                if (ImGui::BeginTable("CoefficientsTable", 5, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg)) {
                    ...
                    for (size_t i = 0; i < rowCount; ++i) {
                        ...
                        // 2. Check if it's a seasonal feature
                            ...
                            // Try to extract seasonal value from feature name
                            if (seasonalPos != std::string::npos && seasonalPos + 9 < featureName.size()) {
                                std::string seasonalStr = featureName.substr(seasonalPos + 9);
                                try {
                                    int seasonalValue = std::stoi(seasonalStr);
                                    ImGui::Text("Seasonal %d", seasonalValue);
                                } catch (...) {
                                    // If extraction fails, use the global seasonality value
                                    if (seasonality_ > 0) {
                                        ImGui::Text("Seasonal %d", seasonality_);
                                    } else {
                                        ImGui::Text("Seasonal");
                                    }
                                }
                            } else {
                                // If no seasonal suffix, use the global seasonality value
                                if (seasonality_ > 0) {
                                    ImGui::Text("Seasonal %d", seasonality_);
                                } else {
                                    ImGui::Text("Seasonal");
